...
describe('useOrganizationData Hook', () => {
  ...
  it('초기 상태가 올바르게 설정되어야 한다', async () => {
    ...
    expect(result.current.teams).toEqual(initialTeamsData);
  ...
  it('통계 계산 기능이 제공되어야 한다', async () => {
    ...
    const calculatedTotal = result.current.stats.active + 
  ...
  it('확인 다이얼로그 상태가 포함되어야 한다', () => {
    ...
    expect(result.current.confirmation.isVisible).toBe(false);
  });
